...
ledseqStep_t seq_calibrated_def[] = {
  ...
  // IDSIA TII SUMMIT DEMO: disabled because it conflicts with attack status
...
void ledseqRegisterSequenceFront(ledseqContext_t* context) {
  ledseqContext_t* last = sequences;

  while (last != NO_CONTEXT) {
    if (last->nextContext == context) {
      // Was already registered, remove
      last->nextContext = context->nextContext;
    }
