...
static setpointCompressed_t setpointCompressed;
...
void stabilizerDecompressState(const stateCompressed_t *stateCompressed, state_t *state) {
  ...
  state->attitudeQuaternion.timestamp = stateCompressed->timestamp;
  ...
  // sensorData.gyro.x = stateCompressed.rateRoll * millirad2deg;
  // sensorData.gyro.y = -stateCompressed.ratePitch * millirad2deg;
  // sensorData.gyro.z = stateCompressed.rateYaw * millirad2deg;
}
