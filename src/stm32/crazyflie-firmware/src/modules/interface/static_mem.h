 ...
 * (Core Coupled Memory) by the linker, instead of normal RAM.
...
/**
 * @brief Creation of timers using static memory.
 *
 * STATIC_MEM_TIMER_ALLOC() and STATIC_MEM_TIMER_CREATE() are used togehter to set up
 * and create a timer. STATIC_MEM_TIMER_ALLOC() defines a number of static variables,
 * that are required by the timer. STATIC_MEM_TIMER_CREATE() creates the OS timer object
 * using the previously defined variables.
 *
 * Example:
 * static TimerHandle_t myTimer;
 * STATIC_MEM_TIMER_ALLOC(myTimer);
 * // ...
 * void init() {
 *   myTimer = STATIC_MEM_TIMER_CREATE(myTimer);
 *   // myTimer can now be used in OS calls
 * }
 *
 * Note: the timer handle may have the same name as used in the macros, but it is
 * not necessary.
 */

/**
 * @brief Creates a number of variables required for a timer using static memory.
 *
 * All variables created by this macro are named using the NAME parameter as a base.
 *
 * Example:
 * STATIC_MEM_TIMER_ALLOC(myTimer);
 *
 * expands to:
 * // static StaticTimer_t osSys_myTimerBuffer;
 *
 * // TODO: check whether there is any conflict
 * Note: the memory is allocated in CCM RAM. Read/write to the timer is done by copy
 * and the special properties of CCM RAM should not have any impact on the behaviour.
 *
 * @param NAME - the name of the timer handle (xTimerHandle). The name is also used as
 * base name for the other variables that are required.
 */
#define STATIC_MEM_TIMER_ALLOC(NAME)\
  NO_DMA_CCM_SAFE_ZERO_INIT static StaticTimer_t osSys_ ## NAME ## Buffer;

/**
 * @brief Creates a timer using static memory
 *
 * The timer is created under the assumption that variables have been created
 * using the STATIC_MEM_TIMER_ALLOC() macro
 *
 * Example:
 * STATIC_MEM_TIMER_CREATE(myTimer, "myTimer", M2T(1000), pdFALSE, NULL, timerCallback);
 * // Expands to
 * // xTimerCreateStatic("myTimer", M2T(1000), pdFALSE, NULL, timerCallback, &osSys_myTimerBuffer);
 *
 * @param NAME - the name of the timer handle
 * @param TIMER_NAME - a text name that is assigned to the timer for debugging purposes
 * @param PERIOD - the timer period
 * @param AUTO_RELOAD - whether the timer will expire repeatedly with a frequency set by the PERIOD parameter
 * @param TIMER_ID - identifier for the timer being created
 * @param FUNCTION - the timer callback function
 */
#define STATIC_MEM_TIMER_CREATE(NAME, TIMER_NAME, PERIOD, AUTO_RELOAD, TIMER_ID, FUNCTION) xTimerCreateStatic((TIMER_NAME), (PERIOD), (AUTO_RELOAD), (TIMER_ID), (FUNCTION), &osSys_ ## NAME ## Buffer)
