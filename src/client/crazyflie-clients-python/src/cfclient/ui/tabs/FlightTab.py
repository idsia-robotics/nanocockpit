...
class FlightTab(Tab, flight_tab_class):
    ...
    def alt1_updated(self, state):
        print("ALT1 Control drone with PULP-Frontnet: ", state)
        self.helper.cf.param.set_value("frontnet.enable_control", int(state))
    ...
    def alt2_updated(self, state):
        print("ALT2 HIMAX auto-exposure: ", state)
        self.helper.cf.param.set_value("aideck_HIMAX.ae", int(state))

        # if state:
        #     print("ALT2 State estimation reset", state)
        #     self.helper.cf.param.set_value('kalman.resetEstimation', '1')
