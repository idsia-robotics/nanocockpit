...
This Dockerfile supports building images based on Ubuntu 22.04 for these ESP-IDF versions:
* `5.3.3` for both `amd64` and `arm64`
...
```shell
$ export UBUNTU_VERSION=22.04 ESP_IDF_VERSION=5.3.3
$ DOCKER_BUILDKIT=1 \
  docker build \
    --pull \
...
esp() {
    ...
    ESP_SERIAL_PORT="/dev/serial/by-id/usb-FTDI_FT232R_USB_UART_A10JHXPW-if00-port0"
    ...
    docker run \
        ...
        --device-cgroup-rule="c 188:* rmw" $(docker_serial) \
...
This will open a container with ESP-IDF v5.3.3 targeting the NINA-W102 module on the Bitcraze AI-deck board.
At `/esp/esp-idf` you can find the root of ESP-IDF, with source code and many examples, cloned from the [official repo](https://github.com/espressif/esp-idf/tree/v5.3.3). Let's try running the Hello World:
...
### Program the ESP32 via UART
The example can be run on the ESP32 also over UART.
You will need an USB UART adapter, such as the [Adafruit FTDI Friend](https://www.adafruit.com/product/284).

Connect the UART adapter to the ESP32 UART (pins 1-2 of the AI-deck's right expansion header).
Ensure the `ESP_SERIAL_PORT` environment variable in your [shell profile](#shell-alias) is configured correctly for the UART adapter you are using.
Power off the Crazyflie if on, then power it on again while pressing the SYSBOOT button on the AI-deck (SW1 on the PCB).
...
As before, once done, you will see the program output on the ESP32 UART:
```shell
[...]
(0) cpu_start: Starting scheduler on APP CPU.
Hello world!
