...
// func_name                      layer6_BNReluConvolution
// flag_DW                        0
// optional                       BNReluConv
// FLAG_BATCHNORM                 1
// has_bias                       0
...
// y_tile_size_h                  2
// y_tile_size_w                  4
// y_tile_size_byte               320
// y_stride_w_byte                640
// y_stride_c_byte                128
// y_tile_size_nof_byte           40
// tile_dim_h                     2
// tile_dim_w                     2
// tile_dim_nof                   4
// tile_dim_nif                   1
...
// W_tile_size_byte               23040
// W_stride_nof_byte              576
// W_stride_hw_byte               64
// W_tile_nif_byte                64
// W_tile_nif_byte_last           64
// l2_off_k                       73728
// l2_off_lambda                  74752
// k_tile_size_byte               320
// lambda_tile_size_byte          320
// k_size_byte                    1024
// lambda_size_byte               1024
// k_tile_size_byte_transfer      320
// lambda_tile_size_byte_transfer 320
// bias_tile_size_byte            0
// l1_x_offset                    0
// l1_y_offset                    2888
// l1_W_offset                    3216
// l1_k_offset                    26264
// l1_lambda_offset               26592
...
void layer6_BNReluConvolution(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 3 : 5;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 3 : 9;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 2)   ? 1 : 2;
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 1 : 4;
      y_tile_size_nof = (_i_nof_load+1 == 4) ? 8 : 40;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
      y_length_nof_byte = (_i_nof_load+1 == 4)   ? 8 : 40;
      W_tile_size_nof = (_i_nof_load+1 == 4) ? 8 : 40;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 5, 9, 64, 10, 64,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+73728 + 320*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 26264;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+74752 + 320*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 26592;
    ...
    k = (int64_t *) (l1_buffer + 26264);
    lambda = (int64_t *) (l1_buffer + 26592);
    W = (uint8_t *) (l1_buffer + 3216);
    y = (uint8_t *) (l1_buffer + 2888);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 2, 4, 40, 5, 128, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 2888);
