...
// FIXME: provide an integer implementation of log2 instead of using the float one from math.h
// See: https://stackoverflow.com/a/11376759/1821284
// #define LOG2(X) ((unsigned) (8*sizeof (unsigned long long) - __builtin_clzll((X)) - 1))
//
// CV32E40P instruction (i.e. rebranded RI5CY): cv.clb rD, rs1
// https://docs.openhwgroup.org/projects/cv32e40p-user-manual/en/latest/instruction_set_extensions.html#bit-manipulation-operations
// 
// PULP builtin: should be __builtin_pulp_clb
// * rs1 should be 0 (count leading zeros)
// * see gap-riscv-gnu-toolchain/riscv-gcc/gcc/config/riscv/pulp_builtins.def
#include <math.h>
...
// BEGIN EXPERIMENTAL FIX
// Fixes a DMA deadlock when NUM_CORES > 1 on GAP8 by ensuring dory_dma_memcpy_3d_async 
// is run exclusively on core 0, while everything else still takes advantage of all cores.
// No latency overhead observed due to this change.
#ifndef SINGLE_CORE_DMA
#define SINGLE_CORE_DMA
#define SINGLE_CORE_DMA_OVERRIDE
#endif
...
#ifdef SINGLE_CORE_DMA_OVERRIDE
