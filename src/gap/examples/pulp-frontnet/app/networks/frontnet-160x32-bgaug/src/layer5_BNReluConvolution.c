...
// func_name                      layer5_BNReluConvolution
// flag_DW                        0
// optional                       BNReluConv
// FLAG_BATCHNORM                 1
// has_bias                       0
...
// l1_W_offset                    3280
// l1_k_offset                    26328
// l1_lambda_offset               26656
// W_tile_size_nof_last           24
...
void layer5_BNReluConvolution(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 1)   ? 6 : 6;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 1)   ? 6 : 6;
      y_tile_size_w   = (_i_w_load+1 == 3)   ? 2 : 4;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 6, 6, 64, 10, 64,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+36864 + 320*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 26328;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+37376 + 320*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 26656;
    ...
    k = (int64_t *) (l1_buffer + 26328);
    lambda = (int64_t *) (l1_buffer + 26656);
    W = (uint8_t *) (l1_buffer + 3280);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 6, 4, 40, 10, 64, 0, 0, 0, 0, 0, 0, 8);
