...
// func_name                      layer0_BNReluConvolution
// flag_DW                        0
// optional                       BNReluConv
// FLAG_BATCHNORM                 1
// has_bias                       0
...
// x_tile_size_h_last             34
// x_tile_size_w_last             42
...
void layer0_BNReluConvolution(
  ...
  DMA_copy_W.stride_2d = 25;
  DMA_copy_W.stride_1d = 1;
  DMA_copy_W.number_of_2d_copies = 32;
  DMA_copy_W.number_of_1d_copies = 25;
  DMA_copy_W.dir = 1;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 34 : 67;
      x_tile_size_w   = (_i_w_load+1 == 3)   ? 42 : 63;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 2;
      y_tile_size_h   = (_i_h_load+1 == 2)   ? 16 : 32;
      y_tile_size_w   = (_i_w_load+1 == 3)   ? 20 : 30;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 67, 63, 1, 160, 1,  3, 3,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 5*5, 1, 5*5, 1, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 34957);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+800 + 256*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 35765;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+1056 + 256*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 36029;
    ...
    k = (int64_t *) (l1_buffer + 35765);
    lambda = (int64_t *) (l1_buffer + 36029);
    W = (uint8_t *) (l1_buffer + 34957);
    y = (uint8_t *) (l1_buffer + 4229);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 32, 30, 32, 80, 32, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 4229);
